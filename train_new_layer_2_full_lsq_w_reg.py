...
def main():
    ...
    if args.model == 'resnet18':
        model = resnet_lsq_full.resnet18(nbits_a=args.num_bits, nbits=args.num_bits).to(args.device)
        model_copy = resnet_lsq_full.resnet18(nbits_a=args.num_bits, nbits=args.num_bits).to(args.device)
    ...
    for (names, params) in model.named_parameters():
        if params.requires_grad and 'conv' in names and 'bias' not in names:
            print(names)
            if 'alpha' in names:
                print(params)
    ...
    #R_matrix = torch.zeros(num_layers_to_quantize, 1).to(device)
    R_matrix = []
    k = 0
    for (names, params) in model.named_parameters():
        if params.requires_grad and 'alpha' in names and 'downsample' not in names and 'actq' not in names:
            print(names)
            R_matrix.append(params.detach())
            k += 1
    R_matrix = torch.stack(R_matrix, dim=0).to(device)
    
    #S_vec_np = np.arange(2**(args.num_bits)) - ((2**(args.num_bits)) - 1)/2
    S_vec_np = np.arange(2**(args.num_bits)) - (2**(args.num_bits - 1))
    S_vec = torch.from_numpy(np.float32(S_vec_np)).to(device)
    S_matrix = torch.zeros(2**(args.num_bits), 1).to(device)
    S_matrix[:, 0] = S_vec
    ############################################################################
    ...
    for epoch in range(args.num_epochs): 
        ...
        for i, (inputs, labels) in enumerate(trainloader, 0):
            # Copy alpha values into R_matrix
            k = 0
            for (names, params) in model.named_parameters():
                if params.requires_grad and 'alpha' in names and 'downsample' not in names and 'actq' not in names:
                    R_matrix[k, 0] = params[0].detach()
                    k += 1
        ...
        if ((epoch + 1) % args.epoch_log_test == 0):
            with torch.no_grad():
                model_copy.load_state_dict(model.state_dict()) #No need to manually quantize weights in model_copy as the model does it intrinsically
                top5_acc_compute = torchmetrics.Accuracy(task='multiclass', num_classes=num_classes, top_k=5).to(device)
                for i, (inputs, labels) in enumerate(testloader, 0):
                    ...
                    if (args.fixed_levels or args.learnable_levels):
                        ...
                        output = model_copy(inputs) #No change - this mode is for pure LSQ training
                        #output = model.forward_no_quant(inputs)
    ...
    if args.save_model and (args.learnable_levels or args.fixed_levels):
        ...
        if args.use_grad_scale:
            torch.save(model_copy.state_dict(), "./new_models/" + str(args.dataset) + '_' + str(args.model) + '_full_lsq_w_reg_grad_scale_' + str(args.num_bits) + "bits.ckpt")
            np.savez("./new_models/" + str(args.dataset) + '_' + str(args.model) + '_full_lsq_w_reg_grad_scale_' + str(args.num_bits) + "bits_RS_mats.npz", R_matrix=R_matrix_np, S_matrix=S_matrix_np)
        else:
            torch.save(model_copy.state_dict(), "./new_models/" + str(args.dataset) + '_' + str(args.model) + '_full_lsq_w_reg_' + str(args.num_bits) + "bits.ckpt")
            np.savez("./new_models/" + str(args.dataset) + '_' + str(args.model) + '_full_lsq_w_reg_' + str(args.num_bits) + "bits_RS_mats.npz", R_matrix=R_matrix_np, S_matrix=S_matrix_np)
