...
def main():
    ...
    if args.save_model and args.learnable_levels:
        ...
        if args.use_grad_scale:
            torch.save(model_copy.state_dict(), "./new_models/" + str(args.dataset) + '_' + str(args.model) + '_lsq_w_reg_grad_scale_' + str(args.num_bits) + "bits.ckpt")
            np.savez("./new_models/" + str(args.dataset) + '_' + str(args.model) + '_lsq_w_reg_grad_scale_' + str(args.num_bits) + "bits_RS_mats.npz", R_matrix=R_matrix_np, S_matrix=S_matrix_np)
        else:
            torch.save(model_copy.state_dict(), "./new_models/" + str(args.dataset) + '_' + str(args.model) + '_lsq_w_reg_' + str(args.num_bits) + "bits.ckpt")
            np.savez("./new_models/" + str(args.dataset) + '_' + str(args.model) + '_lsq_w_reg_' + str(args.num_bits) + "bits_RS_mats.npz", R_matrix=R_matrix_np, S_matrix=S_matrix_np)
