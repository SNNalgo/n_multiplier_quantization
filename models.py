...
class VGG13_Act(nn.Module):
    ...
    def infer_traditional_act(self, x, num_bits, scale_values, quantize_fl):
        ...
        for layer in self.classifier:
            ...
            if not quantize_fl:
                # dont quantize last layer
                if isinstance(layer, nn.ReLU):
                    if count == len(scale_values)-1:
                        x = x
                        count += 1
                    else:
                        x = quantize_traditional(x, num_bits, scale_values[count], signed=False)
    ...
    def infer_layer(self, x, allowed_acts, num_layers_activation, quantize_fl, device):
            ...
            if not quantize_fl:
                ...
                if isinstance(layer, nn.ReLU):
                    if count == 0:
                        ...
                        x = quantize_general(x, allowed_acts[count], device)
                        count += 1
        ...
        for layer in self.classifier:
            ...
            if not quantize_fl:
                # dont quantize last layer
                if isinstance(layer, nn.ReLU):
                    if count == num_layers_activation-1:
                        x = x
                        count += 1
                    else:
                        x = quantize_general(x, allowed_acts[count], device)
