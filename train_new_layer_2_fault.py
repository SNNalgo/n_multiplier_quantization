        ...
        for (names, params) in model.named_parameters():
            if params.requires_grad and 'bn' not in names and 'downsample' not in names and 'bias' not in names:
                if count == 0 or count == num_layers_to_quantize+1:
                    ...
                    #params_bit_fault_list.append(params_bit_fault)
    ...
    for epoch in range(args.num_epochs): 
                        ...
                        if params.requires_grad and 'bn' not in names and 'downsample' not in names and 'bias' not in names:
                            if count == 0 or count == num_layers_to_quantize+1:
                                ...
                                if args.use_mse_loss:
                                    ...
                                    reg_loss_w += reg_loss_current*q_p/(num_params_current)
                                    count += 1
                                    k += 1
                                else:
                                    if args.fault_aware:
                                        reg_loss_current, num_params_current = get_closest_loss_w_fault(params, allowed_weight[k], reg_sig_w_values[k], params_lvl_valid_list[k])
                                    else:
                    ...
                    for (names, params) in model.named_parameters():
                        if params.requires_grad and 'bn' not in names and 'downsample' not in names and 'bias' not in names:
                            if args.use_mse_loss:
                                ...
                                if args.fault_aware:
                                    reg_loss_current, num_params_current = get_closest_loss_w_fault(params, allowed_weight[count], reg_sig_w_values[count], params_lvl_valid_list[count])
                                else:
        ...
        if ((epoch + 1) % args.epoch_log_test == 0):
            with torch.no_grad():
                        ...
                        for (names, params) in model_copy.named_parameters():
                            if params.requires_grad and 'bn' not in names and 'downsample' not in names and 'bias' not in names:
                                if count == 0 or count == num_layers_to_quantize+1:
                                    ...
                                    for bit in range(args.num_bits):
                                        params_bit = param_inds_copy%2
                                        if bit==0:
                                            new_param = params_bit*params_bit_fault_list[k][bit]
                                            #new_param = params_bit
                                        else:
                                            new_param = new_param + (2**bit)*params_bit*params_bit_fault_list[k][bit]
                                            #new_param = new_param + (2**bit)*params_bit
                                        param_inds_copy = param_inds_copy//2
                                    ...
                                    print('max diff of new_param and old_param_inds: ', torch.max(torch.abs(new_param-old_param_inds)))
                                    print('max of params.data: ',torch.max(params.data))
                                    print('min of params.data: ',torch.min(params.data))
                        ...
                        for (names, params) in model_copy.named_parameters():
                            if params.requires_grad and 'bn' not in names and 'downsample' not in names and 'bias' not in names:
                                ...
                                for bit in range(args.num_bits):
                                    params_bit = param_inds_copy%2
                                    if bit==0:
                                        new_param = params_bit*params_bit_fault_list[count][bit]
                                    else:
                                        new_param = new_param + (2**bit)*params_bit*params_bit_fault_list[count][bit]
                                    param_inds_copy = param_inds_copy//2
                ...
                # calculate scale factors for activations using the first 2 train bacthes
                    ...
                    for i, (inputs, labels) in enumerate(trainloader, 0):
                        ...
                        if i == 0:
                            ...
                            for act in activations:
                                ...
                                print("max value which is q_p*s:", q_p*scale_factor)
                                print("actual max value:", torch.max(act))
                ...
                if (epoch<100 and epoch%5==4) or (epoch>=100 and epoch%4==3):
