...
def main():
    ...
    if args.model == 'resnet18':
        ...
        if args.use_pretrained:
            ...
            #pretrained_model_path = "./saved_models/imagenet_resnet18_non_quantized.ckpt"
            model.load_state_dict(torch.load(pretrained_model_path), strict=False)

    if args.model == 'resnet50':
        model, _ = resnet_lsq.resnet50(nbits_a=args.num_bits).to(args.device)
        model_copy, _ = resnet_lsq.resnet50(nbits_a=args.num_bits).to(args.device)

        if args.use_pretrained:
            pretrained_model_path = "./saved_models/imagenet_resnet50_lsq_act_non_quantized_for_" + str(args.num_bits) + "bits.ckpt"
            model.load_state_dict(torch.load(pretrained_model_path), strict=False)
    ...
    if args.save_model and not args.learnable_levels and not args.fixed_levels:
        torch.save(model.state_dict(), "./saved_models/" + str(args.dataset) + '_' + str(args.model) + '_' + str(args.num_epochs) + '_epochs_lsq_act_non_quantized_for_' + str(args.num_bits) + "bits.ckpt")
    
    if args.save_model and args.learnable_levels:
        if args.use_grad_scale:
            torch.save(model_copy.state_dict(), "./new_models/" + str(args.dataset) + '_' + str(args.model) + '_lsq_act_grad_scale_learnable_quantized_' + str(args.num_bits) + "bits.ckpt")
        else:
            torch.save(model_copy.state_dict(), "./new_models/" + str(args.dataset) + '_' + str(args.model) + '_lsq_act_learnable_quantized_' + str(args.num_bits) + "bits.ckpt")
    
    if args.save_model and args.fixed_levels:
        if args.use_grad_scale:
            torch.save(model_copy.state_dict(), "./new_models/" + str(args.dataset) + '_' + str(args.model) + '_lsq_act_grad_scale_fixed_quantized_' + str(args.num_bits) + "bits.ckpt")
        else:
            torch.save(model_copy.state_dict(), "./new_models/" + str(args.dataset) + '_' + str(args.model) + '_lsq_act_fixed_quantized_' + str(args.num_bits) + "bits.ckpt")
