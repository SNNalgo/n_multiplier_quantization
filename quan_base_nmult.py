...
def get_S_matrix(n):
    ...
    # initialize the matrix to be size 2^n x (n) where n is nbits of quantization
    S = np.zeros((2**n, n))

    # set the columns to be the reverse binary representation of the row number
    for i in range(2**n):
...
class _Conv2dQ(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
        ...
        if self.nbits < 0:
            ...
            self.register_buffer('base_vals', None)
            return
        self.q_mode = kwargs_q['mode']
        '''
        if self.q_mode == Qmodes.kernel_wise:
            self.register_buffer('alpha', torch.Tensor(out_channels, self.nbits))
            self.register_buffer('S', torch.tensor(get_S_matrix(self.nbits), dtype=torch.float32))
        else:  # layer-wise quantization
            self.register_buffer('alpha', torch.Tensor(self.nbits))
            self.register_buffer('S', torch.tensor(get_S_matrix(self.nbits), dtype=torch.float32))
        '''
        #Only doing layer-wise for now
        self.register_buffer('alpha', torch.Tensor(self.nbits))
        self.register_buffer('S', torch.tensor(get_S_matrix(self.nbits), dtype=torch.float32))
        self.register_buffer('base_vals', 2 ** torch.arange(self.nbits))
