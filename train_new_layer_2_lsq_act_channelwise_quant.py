...
parser.add_argument('--load_q_wts', action='store_true', help='load quantized weights into model')
...
def main():
        ...
        for (names, params) in model.named_parameters():
            if params.requires_grad and 'bn' not in names and 'downsample' not in names and 'bias' not in names and 'actq' not in names:
                if count == 0 or count == num_layers_to_quantize+1:
                    ...
                    R_vec_expanded_shape = (params_np_shape[-1], args.num_bits+1)
                    R_vec_expanded_shape_2 = params_np_shape + (args.num_bits+1,)
                    
                    R_vec_expanded = np.ones(R_vec_expanded_shape)
                    full_weight_ones = np.ones(R_vec_expanded_shape_2)
                    
                    R_vec_expanded = torch.from_numpy(np.float32(R_vec_expanded)).to(device)
                    full_weight_ones = torch.from_numpy(np.float32(full_weight_ones)).to(device)
                    ...
                    R_vec = torch.tensor(get_R_vector(args.num_bits, max_limit), dtype=torch.float32).to(device)
                    R_vec_expanded = R_vec_expanded * R_vec
                    R_vec_expanded_2 = (full_weight_ones * R_vec_expanded).to(device)
                    allowed_weight.append(torch.matmul(R_vec_expanded_2, S_matrix.T).to(device))
                    R_vecs.append(R_vec_expanded)
                    full_weight_ones_list.append(full_weight_ones)
    ...
    if args.learnable_levels:
        for layer_n in range(len(R_vecs)):
            R_vecs[layer_n] = nn.Parameter(R_vecs[layer_n])

        optimizer_levels_r = optim.SGD(R_vecs, lr=args.lr_levels_r, momentum=args.momentum, weight_decay=args.weight_decay)
    ...
    for epoch in range(args.num_epochs): 
        ...
        for i, (inputs, labels) in enumerate(trainloader, 0):
            #allowed_weight = torch.matmul(R_matrix, S_matrix.T).to(device)
            for layer_n in range(len(R_vecs)):
                R_vec_expanded_2 = (full_weight_ones_list[layer_n] * R_vecs[layer_n])
                allowed_weight[layer_n] = torch.matmul(R_vec_expanded_2, S_matrix.T)
                        ...
                        if params.requires_grad and 'bn' not in names and 'downsample' not in names and 'bias' not in names and 'actq' not in names:
                            if count == 0 or count == num_layers_to_quantize+1:
                                ...
                                if args.use_grad_scale and (epoch>args.start_lambda_schedule_epoch):
                                    ...
                                    #reg_loss_w += reg_loss_current*q_p*torch.sqrt(q_p/num_params_current)             # trying new
                                count += 1
                                k += 1
                else:
                    count = 0
        ...
        if ((epoch + 1) % args.epoch_log_test == 0):
            ...
            with torch.no_grad():
                ...
                if args.load_q_wts and (epoch>=2*args.num_epochs/3 and epoch%3==0):
