...
class ResNet(nn.Module):
    ...
    def forward_act(self, x):
        ...
        for layer in self.layer1:
            out, inter_act = layer.forward_act(out)
    ...
    def forward_clamp(self, x, allowed_act):
        ...
        for layer in self.layer1:
            out, inter_act = layer.forward_clamp(out, allowed_act)
            activations.append(inter_act)
            activations.append(out)
    ...
    def infer(self, x, allowed_act):
        ...
        for layer in self.layer1:
            out = layer.infer(out, allowed_act)
    ...
    def infer_traditional_act(self, x, num_bits, scale_factors_activations, quantize_fl):
        ...
        if not quantize_fl:
            ...
            out = quantize_traditional(out, num_bits, scale_factors_activations[count], signed=False)
        
        count += 1

        for layer in self.layer1:
            out = layer.infer_traditional_act(out, num_bits, scale_factors_activations[count], scale_factors_activations[count+1])
            count += 2
        ...
        for layer in self.layer4:
            # for the last layer, we don't need to quantize the output
            if layer == self.layer4[-1] and not quantize_fl:
                out = layer.infer_traditional_act(out, num_bits, scale_factors_activations[count], scale_factors_activations[count+1], quantize_last=False)
                count += 2
            else:
                out = layer.infer_traditional_act(out, num_bits, scale_factors_activations[count], scale_factors_activations[count+1])
                count += 2
