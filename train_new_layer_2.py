...
parser.add_argument('--lr_levels_r', default=1e-6, type=float, help='learning rate for training the linear classifier')
...
parser.add_argument('--no_act', action='store_true', help='activation quantization not done in training')
...
def main():
    ...
    if args.save_model and args.learnable_levels:
        torch.save(model.state_dict(), "./new_models/" + str(args.dataset) + '_' + str(args.model) + '_' + 'learnable_fp' + '_' + str(args.num_bits) + "bits.ckpt")
        torch.save(model_copy.state_dict(), "./new_models/" + str(args.dataset) + '_' + str(args.model) + '_' + 'learnable_quantized' + '_' + str(args.num_bits) + "bits.ckpt")
    
    if args.save_model and args.fixed_levels:
        torch.save(model.state_dict(), "./new_models/" + str(args.dataset) + '_' + str(args.model) + '_' + 'fixed_fp' + '_' + str(args.num_bits) + "bits.ckpt")
        torch.save(model_copy.state_dict(), "./new_models/" + str(args.dataset) + '_' + str(args.model) + '_' + 'fixed_quantized' + '_' + str(args.num_bits) + "bits.ckpt")
    # if args.learnable_levels:
